...
func (s *PublicBlockChainAPI) EstimateGasManyTx(ctx context.Context, args []TransactionArgs,
	...
	msg, err := args[len(args)-1].ToMessage(s.b.RPCGasCap(), header.BaseFee)
	if err != nil {
		return nil, err
	}
	...
	var rGas, rErr []interface{}
	for _, a := range args {
		//func DoEstimateGas2(ctx context.Context, evm *vm.EVM, vmError func() error, state *state.StateDB, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, gasCap uint64) (hexutil.Uint64, error) {
		eGas, err := DoEstimateGas2(ctx, evm, vmError, state, header, s.b, a, bNrOrHash, s.b.RPCGasCap())
		rGas = append(rGas, eGas)
		if err != nil {
			rErr = append(rErr, err.Error())
		} else {
			rErr = append(rErr, "")
		}
	}
	return []interface{}{rGas, rErr}, nil
}
